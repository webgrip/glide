...
4. A trusted controller-side reconciliation caller may invoke `Store.ReconcileLLMAccount` with the Run identity, final amount and evidence reference. This store operation is not exposed as a worker or unauthenticated HTTP settlement endpoint. The current baseline requires an operational integration for that trusted call.
