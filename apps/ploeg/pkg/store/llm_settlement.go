...
func (s *Store) UnsettledLLMAccounts(ctx context.Context, after int64, quietFor time.Duration, limit int) ([]UnsettledLLMAccount, error) {
	...
	rows, err := s.pool.Query(ctx, `SELECT r.id,a.run_token,a.gateway_key_id,a.alias,a.state,a.updated_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v.RunID, &v.RunToken, &v.GatewayKeyID, &v.Alias, &v.State, &v.QuietSince, &v.MintBegan); err != nil {
			return nil, err
		}
