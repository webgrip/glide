...
function verifyConditions(_pluginConfig, { branch, options }) {
  ...
  if (options?.tagFormat !== 'ploeg-v${version}') {
    throw new Error('Ploeg release tags must use ploeg-v${version}.');
  }
}
...
function verifyRelease(pluginConfig, context) {
  ...
  if (release.channel !== 'development' || release.gitTag !== `ploeg-v${release.version}`) {
    throw new Error('Ploeg release version, development channel and ploeg-v-prefixed tag must agree.');
  }
}
