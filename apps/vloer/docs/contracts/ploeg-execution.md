...
This is the opt-in execution contract implemented by [the authority client](../../src/execution-authority.ts) and [the session engine](../../src/engine.ts). The [operating guide](../../../../docs/workflows/managed-execution.md) covers configuration and qualification. Standalone sessions retain their existing contract when `execution` is absent. A session becomes managed when De Vloer persists its admission intent, before it calls Ploeg. From then on it cannot fall back to standalone execution, even if the admission response was lost or `execution` is later removed: start, resume, retry and budget increases fail with `authority_required`, `new_authorization_required` or `authority_budget`, and a restart marks it blocked. Restoring the Ploeg connection reconciles it by replaying the same admission. Cancelling it discards it.
