...
**BAND: a proprietary agent-to-agent interaction layer, surveyed 18 September 2026.** [BAND](https://www.band.ai/) sells chat rooms where agents and humans participate together, with `@mention` routing, bilateral Contacts and a delegated token exchange. It is not a protocol anyone else implements: the platform is closed Elixir, SaaS-only at every published price point, and its published [OpenAPI document](https://docs.band.ai/openapi.json) carries zero occurrences of spend, provenance, repository or merge vocabulary across 56 paths. It speaks MCP, A2A and Zed's ACP as SDK adapters, and **zero mentions of AHP appear in its entire documentation corpus**. Rejected as a dependency and mined for design in [the dossier](2026-09-18-band-and-the-interaction-layer.md); the reason is seam fit, since neither application has an agent-to-agent channel at all. Note the naming hazard: BAND's "ACP" is Zed's Agent **Client** Protocol, pinned as `agent-client-protocol>=0.11.0`, not IBM's former Agent **Communication** Protocol.

**AP2 and delegated-authority credentials: prior art for our budget seam.** [AP2 v0.2.0](https://ap2-protocol.org/) (donated to the FIDO Alliance on 2026-04-28) expresses scope, a time bound, a budget cap and an agent binding in one user-signed SD-JWT credential, and lacks only revocation. `draft-nelson-agent-delegation-receipts-10` expresses all five, including a revocation check as the first verification step, against a tamper-evident append-only log. Neither is adoptable here, and both are the closest published design precedent for [Ploeg ADR-0012](https://forgejo.webgrip.dev/webgrip/ploeg/src/branch/development/docs/adrs/0012-two-level-budgets-authorized-and-settled.md). Separately, `draft-ietf-wimse-aims-00` was adopted by the IETF WIMSE working group on 2026-09-15, the first working-group-adopted agent identity document; it composes WIMSE, OAuth 2.0 and SPIFFE rather than inventing, and carries no budget concept.
