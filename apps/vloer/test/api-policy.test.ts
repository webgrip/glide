...
test('a model served outside the allowed providers is refused before the first turn', async t => {
  ...
  server.app.engine.broker = {
    mint: async (session: Session) => ({ key: 'sk-session-fixture-key', alias: 'a', reference: `de-vloer-${session.id}-x`, budgetUsd: session.budgetUsd }), revoke: async () => {}, spend: async () => 0,
...
test('a request the gateway attributes to a provider or region outside policy stops the session and revokes its key', async t => {
  ...
  const { cookie } = await login(server.url);
  server.app.engine.broker = {
    mint: async (session: Session) => ({ key: 'sk-session-fixture-key', alias: 'a', reference: `de-vloer-${session.id}-x`, budgetUsd: session.budgetUsd }), revoke: async (reference: string) => { revoked.push(reference); }, spend: async () => 0.02,
