...
test('live operator projection reads all lanes, complete evidence and honest unknown costs with bounded caching', async t => {
  ...
  assert.equal(upstreamApi.seen.length, 5);
  await ploeg.overview(admin);
  assert.equal(upstreamApi.seen.length, 5);
  await ploeg.overview(admin, undefined, true);
  assert.equal(upstreamApi.seen.length, 10);
  upstreamApi.rotate();
  await ploeg.overview(admin);
  assert.equal(upstreamApi.seen.length, 15, 'rotating the consumer credential invalidates cached scope');
