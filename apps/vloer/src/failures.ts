...
const descriptions: Record<FailureCategory, { message: string; remediation: string }> = {
  ...
  runaway: { message: 'A role exceeded its tool-call limit without finishing.', remediation: 'Read the transcript for what it kept doing. Sharpen the brief or raise the role\'s tool-call limit, then try again.' },
