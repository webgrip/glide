...
type Fake = { base: string; tokens: Record<string, unknown>[]; revoked: string[]; users: number; expiresIn: number; close(): Promise<void> };

async function gitlab(): Promise<Fake> {
  const fake: Fake = { base: '', tokens: [], revoked: [], users: 0, expiresIn: 7200, close: async () => {} };
  let issued = 0;
...
test('a link starts with PKCE, completes against the account, and yields clone access for that host only', async t => {
  ...
  const url = new URL(links.begin('user-1'));
  ...
  await assert.rejects(links.complete('good-code', 'not-a-state'), (error: any) => error instanceof LinkError && error.code === 'link_state');
  const completed = await links.complete('good-code', state);
  ...
  assert.equal(await links.complete('good-code', state).catch((error: any) => error.code), 'link_state');
...
test('an expiring link is refreshed before the clone and revocation forgets and revokes both tokens', async t => {
  const { links, store, fake } = await harness(t);
  fake.expiresIn = 60;
  const state = new URL(links.begin('user-1')).searchParams.get('state')!;
  await links.complete('good-code', state);
...
test('a rejected code and a broken refresh are failures the session can explain', async t => {
  ...
  const state = new URL(links.begin('user-1')).searchParams.get('state')!;
  await assert.rejects(links.complete('bad-code', state), (error: any) => error instanceof RuntimeFailure && error.category === 'workspace_setup' && error.httpStatus === 400);
  fake.expiresIn = 60;
  const again = new URL(links.begin('user-1')).searchParams.get('state')!;
  await links.complete('good-code', again);
  const record = (links as any).record('user-1');
...
test('an unconfigured link describes itself and refuses to start', async t => {
  ...
  assert.throws(() => links.begin('user-1'), (error: any) => error instanceof LinkError && error.status === 409);
