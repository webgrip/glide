...
Ploeg-free operation does not promise offline inference, repository-free chat, or a single binary. Existing standalone Vloer is the starting point. The [architecture](../apps/vloer/docs/architecture.md), [Ploeg execution contract](../apps/vloer/docs/contracts/ploeg-execution.md) and [workspace placement decision](../apps/vloer/docs/adrs/0009-workspace-placement-is-a-session-choice.md) describe its current boundaries.

## Prove the runner boundary first

Compare the [Vloer engine](../apps/vloer/src/engine.ts) and [runtime adapters](../apps/vloer/src/runtime) with [Ploeg's worker](https://forgejo.webgrip.dev/webgrip/ploeg/src/branch/development/pkg/worker/worker.go) and [harness contract](https://forgejo.webgrip.dev/webgrip/ploeg/src/branch/development/pkg/harness/contract.go). Use a small fixture with no paid model calls initially.
...
The [audit](../apps/vloer/docs/research/2026-09-12-documentation-audit.md) records the first pass. Its follow-up records remaining contradictions, changes and validation. These audit records explain the cleanup; the current guides remain the place to learn the system.
