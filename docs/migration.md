...
3. Add Glide to Git-managed CI secret and Renovate repository selection. The [secret reconciler](https://forgejo.webgrip.dev/webgrip/homelab-cluster/src/branch/main/kubernetes/apps/forgejo/forgejo-actions-secrets/app/forgejo-actions-secrets.cronjob.yaml) now scopes the Open VSX token to `glide`, and the [OpenBao bootstrap configuration](https://forgejo.webgrip.dev/webgrip/homelab-cluster/src/branch/main/kubernetes/apps/security/openbao/bootstrap/config.sh) lists `webgrip/glide` in the signing role. The [release preview](operations/first-cutover.md) preflight confirms both from a Glide workflow; a manifest edit alone proves neither. Preserve vault ownership of secret values.
