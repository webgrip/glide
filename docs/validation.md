...
| Operator browser flow | `npm run test:browser` after installing Playwright Chromium | PASS on 2026-09-09 with Chromium 152.0.7977.0: task preview/import/start, immutable bundle/patch/manifest downloads, duplicate import, changed-revision handling, inert tracker text, existing demo/lifecycle flows, a deterministic delayed-response control race, desktop and 390px layout, login/logout; no script/CSP errors |
