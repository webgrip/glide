...
Every staged Markdown page is available at its `.md` path. Structured YAML and JSON remain available alongside it. The default reading bundle stays small. History pages remain reachable through explicit links: research and evidence, ADR records, design baselines, planning and backlog pages, and OpenSpec. The [staging step](../../scripts/docs.py) classifies them by path in [the history rules](../../scripts/docs-rules.py). It gives each staged copy front matter `search: {exclude: true}` and a one-line banner, "Record from <date>; not current guidance." MkDocs and Zensical honor that front matter; the output check hides the same pages from Pagefind and fails if one reaches a search index. The source files stay unchanged. ADR index pages are the current view of each ledger and stay searchable. An LLM index helps discovery; it does not prove that a statement is current.
...
2. Run `mise run docs-check`. Besides generated-view drift, it fails on a missing link target or heading anchor, on a current page that links through a [moved path](../research/2026-09-12-glide-document-paths.json), on a current page that is neither in the `mkdocs.yml` nav nor linked from a nav page, on an inconsistent ADR ledger (system, Ploeg or Vloer), and on a stale [decision register](../reference/decisions.md). Regenerate that register with `mise run docs-decisions`. Run `mise run docs-site-check` to exercise Zensical, both machine-readable outputs, Pagefind and the secret scan in the same pinned image as CI. Docker must be running and the image available locally. Output is under `.build/` and is not committed.
...
The [Garage bootstrap](https://forgejo.webgrip.dev/webgrip/homelab-cluster/src/branch/main/kubernetes/apps/garage/garage/bootstrap/techdocs.job.yaml) provisions `docs-glide` and `docs-glide-trash`. Its scoped key is mirrored through OpenBao to the [Forgejo bridge](https://forgejo.webgrip.dev/webgrip/homelab-cluster/src/branch/main/kubernetes/apps/forgejo/forgejo-actions-secrets/app/forgejo-actions-secrets.cronjob.yaml), which maintains Glide's two `TECHDOCS_S3_*` secrets. The publisher writes under `docs-glide/glide`; the [HTTP route](https://forgejo.webgrip.dev/webgrip/homelab-cluster/src/branch/main/kubernetes/apps/garage/docs-site/app/httproute.yaml) preserves `/glide` when serving it. Garage therefore retains the public prefix when it redirects a directory URL to its trailing-slash form. The first root-level upload remains in the bucket as a migration recovery copy.
