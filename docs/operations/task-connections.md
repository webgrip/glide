...
| `executionOwner` | `interactive` for explicitly operated Vloer work, or `ploeg` for a source reserved for unattended Ploeg work |
...
An `executionOwner` declaration is a local routing rule, not a distributed ownership lease. Keep repositories assigned to Ploeg out of Vloer's interactive execution lane. Do not point a live Ploeg poller at an interactive source and assume that the Vloer configuration coordinates the two services. Automated handoff, shared claims, takeover fencing and crash reconciliation belong to the canonical WorkOrder design.

Both the source and its destination repository must permit interactive execution. The server also checks repository ownership for manually created sessions and rechecks it when launching work, so creating an ad hoc session does not bypass a repository reserved for Ploeg. Existing repository registrations without an ownership declaration retain the interactive default; new task source entries require an explicit declaration. Declare repository ownership too when introducing Ploeg alongside the service.
...
| Interactive import blocked | The source and destination repository must belong to the interactive lane. Coordinate the real tracker/Ploeg ownership before changing configuration |
