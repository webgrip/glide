...
const relayWorker = '/usr/local/lib/de-vloer/relay-worker.mjs';
...
export function workspaceManifests(config: AppConfig, session: Session, repository: Repository, credential: Credential, basic: BasicCredentials, managed: Record<string, unknown>, relay?: RelayBinding): KubernetesObject[] {
  ...
  const relayEnv = relay ? [{ name: 'VLOER_RELAY_URL', value: relay.url }, { name: 'VLOER_RELAY_TOKEN', valueFrom: valueFrom('VLOER_RELAY_TOKEN') }, { name: 'VLOER_RELAY_TARGET', value: 'http://127.0.0.1:4096' }] : [];
  const serveCommand = relay ? ['node', relayWorker, '--', 'opencode', 'serve', '--hostname', '127.0.0.1', '--port', '4096'] : ['opencode', 'serve', '--hostname', '0.0.0.0', '--port', '4096'];
  ...
  const pod = { apiVersion: 'v1', kind: 'Pod', metadata, spec: {
    ...
    containers: [{ name: 'agent', image: k.image, imagePullPolicy: k.pullPolicy ?? 'IfNotPresent', securityContext,
      command: serveCommand, workingDir: '/workspace/repository', env: [...env, ...relayEnv], ...(envFrom.length ? { envFrom } : {}),
  ...
  const networkPolicy = { apiVersion: 'networking.k8s.io/v1', kind: 'NetworkPolicy', metadata, spec: {
    ...
    ingress: relay ? [] : [{ from: k.ingressFrom ?? [{ podSelector: { matchLabels: { 'app.kubernetes.io/name': 'de-vloer' } } }], ports: [{ protocol: 'TCP', port: 4096 }] }],
  ...
  return relay ? [secret, pvc, networkPolicy, pod] : [secret, pvc, service, networkPolicy, pod];
}
...
export function candidateExportManifest(config: AppConfig, session: Session, repository: Repository, relay?: RelayBinding): KubernetesObject {
  ...
  return { apiVersion: 'v1', kind: 'Pod', metadata: { name, namespace: k.namespace, labels: { 'app.kubernetes.io/name': 'de-vloer-agent', 'de-vloer/session': name, 'de-vloer/purpose': 'candidate-export' } }, spec: {
    ...
    containers: [{ name: 'candidate-export', image: k.image, imagePullPolicy: k.pullPolicy ?? 'IfNotPresent',
      command: relay ? ['node', relayWorker, '--', 'node', '--input-type=module', '-e', helper + candidateServerProgram] : ['node', '--input-type=module', '-e', helper + candidateServerProgram],
      env: [{ name: 'CANDIDATE_SESSION', value: session.id }, { name: 'CANDIDATE_REPOSITORY', value: repository.id }, { name: 'CANDIDATE_BASE', value: session.workspace?.metadata?.baseSha ?? '' },
        ...
        ...(relay ? [{ name: 'VLOER_RELAY_URL', value: relay.url }, { name: 'VLOER_RELAY_TOKEN', valueFrom: valueFrom('VLOER_RELAY_TOKEN') }, { name: 'VLOER_RELAY_TARGET', value: 'http://127.0.0.1:4096' }] : [])],
...
export class KubernetesWorkspaces {
  ...
  constructor(config: AppConfig, client?: KubernetesClient, relay: WorkerRelay = new WorkerRelay()) {
    if (!config.kubernetes) throw new Error('Kubernetes workspace configuration missing');
    this.config = config; this.kube = config.kubernetes as KubernetesConfig; this.relay = relay;
    ...
    if (this.kube.transport === 'pull' && !this.kube.relayUrl) throw new Error('kubernetes.transport pull requires kubernetes.relayUrl');
  }
  ...
  private relayBinding(sessionId: string): RelayBinding | undefined {
    ...
    return { url: `${this.kube.relayUrl!.replace(/\/$/, '')}${this.relay.basePath}${sessionId}`, token };
  }
  ...
  async prepare(session: Session, repository: Repository, credential: Credential | undefined, signal: AbortSignal, managed: Record<string, unknown>): Promise<Workspace> {
    ...
    const manifests = workspaceManifests(this.config, session, repository, credential, basic, managed, relay);
    try {
  ...
  async captureCandidate(session: Session, repository: Repository): Promise<Candidate> {
    ...
    try {
      const relay = this.relayBinding(session.id);
      await this.client.request(this.path('Pod'), 'POST', candidateExportManifest(this.config, session, repository, relay));
      const transport = relay ? this.relay.fetcher(session.id) : fetch;
      const read = async (path: string, maximum: number): Promise<Buffer> => {
        const response = await transport(new URL(path, relay ? 'http://workspace' : workspace.endpoint), { headers: { authorization: 'Basic ' + Buffer.from(`${basic.username}:${basic.password}`).toString('base64') }, signal: AbortSignal.timeout(15000), redirect: 'error' });
        if (!response.ok || !response.body) throw new Error('Candidate export unavailable');
        const chunks: Buffer[] = []; let size = 0;
        for await (const chunk of response.body) { size += chunk.length; if (size > maximum) { await response.body.cancel().catch(() => {}); throw new Error('Candidate export is too large'); } chunks.push(Buffer.from(chunk)); }
        return Buffer.concat(chunks);
  ...
  async dispose(workspace: Workspace): Promise<void> {
    ...
    this.tokens.delete(workspace.id);
