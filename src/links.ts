...
export class Links {
  ...
  private isConfigured(provider: LinkProvider): boolean { return provider === 'gitlab' ? Boolean(this.gitlab()?.clientId) : Boolean(this.clickup()?.clientId && this.clickup()?.clientSecret); }
  ...
  private record(provider: LinkProvider, userId: string): LinkRecord | undefined { return this.isConfigured(provider) ? this.store.getSecret<LinkRecord>(this.key(provider, userId)) : undefined; }
  ...
  describe(userId: string, provider: LinkProvider = 'gitlab'): PublicLink {
    ...
    if (!record) return { provider, host: this.host(provider), configured: this.isConfigured(provider), linked: false };
    return { provider, host: this.host(provider), configured: true, linked: true, login: record.login, webUrl: record.webUrl, scopes: record.scopes, linkedAt: record.linkedAt, expiresAt: record.expiresAt };
  }

  describeAll(userId: string): PublicLink[] { return linkProviders.filter(provider => this.config.links?.[provider]).map(provider => this.describe(userId, provider)); }

  describeFor(userId: string, repositoryUrl: string): PublicLink | undefined {
    const gitlab = this.gitlab();
    if (!gitlab?.clientId) return undefined;
  ...
  begin(provider: LinkProvider, userId: string): string {
    if (!this.isConfigured(provider)) throw new LinkError(409, 'link_unconfigured', `${provider === 'gitlab' ? 'GitLab' : 'ClickUp'} linking is not configured on this workbench.`);
    const now = Date.now();
  ...
  async complete(provider: LinkProvider, code: string, state: string): Promise<{ userId: string; link: PublicLink }> {
    if (!this.isConfigured(provider)) throw new LinkError(409, 'link_unconfigured', 'This link is not configured on this workbench.');
    ...
    if (provider === 'gitlab') {
      ...
      record = { ...token, verifier: pending.verifier, login: profile.login, webUrl: profile.webUrl, linkedAt: new Date().toISOString() };
    } else {
      ...
      record = { accessToken: data.access_token, scopes: [], login: profile.login, webUrl: appUrl, linkedAt: new Date().toISOString() };
    }
  ...
  async revoke(provider: LinkProvider, userId: string): Promise<void> {
    ...
    if (!record || provider !== 'gitlab') return;
    const { baseUrl, clientId } = this.gitlab()!;
  ...
  async token(userId: string, provider: LinkProvider): Promise<string | undefined> {
    if (provider === 'gitlab') return (await this.access(userId, this.gitlab()?.baseUrl ?? ''))?.password;
    return this.record('clickup', userId)?.accessToken;
  }

  async access(userId: string, repositoryUrl: string): Promise<RepositoryAccess | undefined> {
    const settings = this.gitlab();
  ...
  private async gitlabProfile(baseUrl: string, accessToken: string): Promise<{ login: string; webUrl: string }> {
    let response: Response;
    try {
